...
export default function AgencyEdit() {
	...
	const phoneFaxRule = (deviceType: 'Phone' | 'Fax', num: number) =>
		Yup.string().test(
			`${deviceType}${num}_test`,
			t(`${deviceType} number must contain 10 digits`),
			(value) =>
				value == undefined || (value && /^\d{10}$/.exec(value)) ? true : false
		);
	...
	const validationScheme = Yup.object({
		name: Yup.string()
			...
			.test(
				'name_min_3_letter_test',
				t('The agency name must contain at least 3 letters'),
	...
	return (
		<div className={cn(styles['agency'], styles['edit-mode'])}>
			...
			>
				{({ values, errors }) => formTemplate(values, errors, t)}
			</Formik>
		</div>
	);
}
